...
| Plain Text<br />POST https//open.vidau.ai/v1/video/createTask |
| ------------------------------------------------------------- |
...
| 参数名称            | 类型            | 必填 | 参数描述                                                                                                                                                                                                                                                                                            |
| -------------- | ------------ | -- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| model           | String        | 是  | 模型名称  - Sora 2：sora@2normal  - Sora 2 pro sora@2pro  - Veo 3.1 veo@31normal  - Veo 3.1 fast veo@31fast  - seedance 1.0 pro seedance@1pro  - Seedance 1.0 pro fast seedance@1pro_fast  - Vidu q2 turbo vidu@q2turbo  - Vidu q2 pro vidu@q2pro  - wan 2.5 preview wan@2.5preview |
| taskUUID        | String        | 可选 | 创建任务ID，没有随机初始化，参考值：f5739925-fa8f-41c8-8f33-88aeba1ed365                                                                                                                                                                                                                                         |
| prompt          | String        | 是  | 文本提示词<br />生成视频的文本描述。<br />注1：字符长度不能超过 2000 个字符                                                                                                                                                                                                                                                 |
| negative_prompt | String        | 可选 | 文本负向提示词<br />生成视频的文本描述。<br />注1：字符长度不能超过 2000 个字符                                                                                                                                                                                                                                               |
| image_url       | String        | 可选 | 首帧图像<br />模型将以此参数中传入的图片为首帧画面来生成视频。<br />图片URL（确保可访问）；                                                                                                                                                                                                                                           |
| last_image_url  | String        | 可选 | 尾帧图像<br />模型将以此参数中传入的图片为尾帧画面来生成视频。<br />图片URL（确保可访问）；                                                                                                                                                                                                                                           |
| ref_image_urls  | Array[String] | 可选 | 参考图像<br />模型将以此参数中传入的图片为参考画面来生成视频。<br />图片URL（确保可访问）；                                                                                                                                                                                                                                           |
| generate_audio  | Bool          | 可选 | 是否使用音视频直出能力，默认为false，枚举值为：<br />- false：不需要音视频直出，输出静音视频<br />- true：需要音视频直出，输出带台词以及背景音的视频                                                                                                                                                                                                       |
...
| taskUUID | String | 创建任务ID不为空，则返回原值，创建任务ID为空，则返回随机值 |
| message  | String | 日志信息                            |
...
| Plain Text<br />GET https//open.vidau.ai/v1/queryTask/{taskUUID}/creations |
| -------------------------------------------------------------------------- |
...
| ------ | ----- | -------------------------------------------------------------------------------- |
| code    | String | 生成状态码，创建成功，默认值为200，异常情况，见错误码列表                                                    |
| task    | String | 处理状态：<br />queued  任务排队中<br />running  任务处理中<br />successed 任务成功<br />failed 任务失败 |
| message | String | 日志信息                                                                              |
| result  | Json   | 结果信息                                                                              |
...
| 400       | PageSizeOutOfRange         | page size out of range                     | 图像尺寸有问题。要求：图片大小需小于50M,格式只支持 jpg/jpeg/png/webp, 图片长宽比需要小于14或者41，跳舞特效的图片长宽比需要在 11.2 至 12 之间 |
